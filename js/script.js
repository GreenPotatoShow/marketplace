...
if (localStorage.getItem('count') == undefined) {
    for (let i = 0; i < itemNames.length; i++){
        items[i] = new Item (itemNames[i].innerHTML, itemPrices[i].innerHTML, 0);
    }
    ...
    for (let i = 0; i < itemNames.length; i++){
        items[i] = new Item (itemNames[i].innerHTML, itemPrices[i].innerHTML, localStorage.getItem(i + 'itemCount'));
        if (items[i].count != 0) { 
